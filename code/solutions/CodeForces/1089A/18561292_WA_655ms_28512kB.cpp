...
bool cmp_win(const Sol &a,const Sol &b){
    return a.b>b.b;
}
bool cmp_fail(const Sol &a,const Sol &b){
    return a.a>b.a;
}
...
void print_sol(int wm,int fm,int ws,int fs){
    ...
    if(wm>fm)sort(sols.begin(),sols.end()-1,cmp_win);
    else sort(sols.begin(),sols.end()-1,cmp_fail);
